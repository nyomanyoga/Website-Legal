...
<div id="layanan" class="jumbotron jumbotron-fluid py-4" style="margin-top: 50px;">
    <div class="container">
        <h3>Keputusan Dekan</h3>
        <hr style="width: 24%; border-top: 2px solid white; margin-top: 10px;">
        <div class="row mx-auto">
            <?php $index = count($dekan)-1; ?>
            <?php for ($a = 0; $a <= $index; $a ++) { ?>
                <div class="cads col-md-3">
                    <div class="content card" style="max-width: 15rem; min-height: 7.7rem;">
                      <a href="<?= base_url('detaildekan'); ?>" class="btn my-auto btn-content">
                            <h5 class="icon"><?= $a+1 ?></h5>
                            <h5><?php echo $dekan[$a] ?></h5>
                        </a>
                    </div>
                </div>
            <?php } ?>
